...
export const RoomDetailsCard = ({ roomId, roomName, isActive, refetch }) => {
    ...
    <div className="p-3 flex flex-col w-full bg-background-1000 border rounded-md border-background-100 space-y-2 hover:bg-background-100 duration-200 ">
      <div className="flex w-full     items-center justify-between">
        <p className="flex items-center gap-2">
          Meeting name : <span className="font-semibold">{roomId}</span>{" "}
        </p>

        <Link to="/room-details">Meeting details</Link>
      </div>
      <p>
        Room Name : <span className="font-semibold">{roomName}</span>
      </p>
      {isActive ? (
